...
import { useAccountData, useAccountInfo, useConnection, useGpaConnection } from './connection';
import { useWallet } from '@solana/wallet-adapter-react';
import tuple from 'immutable-tuple';
import { notify } from './notifications';
import BN from 'bn.js';
import { getTokenAccountInfo, parseTokenAccountData, useMintInfos, } from './tokenUtils';
import {
  Balances,
  CustomMarketInfo,
...
import SolapeApi from './solapeConnector';
...
export function useUnmigratedOpenOrdersAccounts() {
  ...
  async function getUnmigratedOpenOrdersAccounts(): Promise<OpenOrders[]> {
    if (!wallet || !connection || !wallet.adapter.publicKey) {
      return [];
    }
...
const _FAST_REFRESH_INTERVAL = 1700;
...
export function useMarkGain() {
  /* todo: enrich with Solape API data for each token and exchange values
   will work on both positive and negative numbers in this format: 0.049 (positive), -0.049 (negative)
   */

  const { name } = useMarket();

  async function getSolapePairs() {
    if (!name) {
      return null;
    }
    let data = await SolapeApi.getPairs();
    let pairVolume = data?.filter(market => {
      // @ts-ignore
...
export function useMarkVolume() {
  /* todo: enrich with Solape API data for each token and exchange values
 1000 divider formatting (US) - might need a reformatting on greater numbers with added k/m/b/t in the future
 (depending on volume of the pair) */

  /* ==================================================
  try stuff with SolapeAPI @ansel LOOK! :see_no_evil:*/
  const { name } = useMarket();
  ...
  return useAsyncData(
    getSolapePairs,
    tuple('getSolapeMarket', name),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
    false,
  );

  /*:see_no_evil_finished
   ================================================== */
...
export function _useUnfilteredTrades(limit = 10000) {
  ...
  const tmpTrades = useAsyncData(
    getUnfilteredTrades,
    tuple('getUnfilteredTrades', market, connection),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
  );
  // console.log("trades: ", tmpTrades);
  const [trades] = tmpTrades;
  return trades;
...
export function useLastDay() {
  ...
  async function getSolapeHistory() {
    if (!name) {
      return null;
    }
    return await SolapeApi.getLastDay(name);
  }

  return useAsyncData(
    getSolapeHistory,
    tuple('getSolapeHistory', name),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
    false,
  );
}

export function useSolapeTrades() {
  const { market, customMarkets } = useMarket();
  const marketAddress = market?.address.toBase58();

  let isCustomMarket = marketAddress && customMarkets.find(cMarket => cMarket.address === marketAddress);

  async function getSolapeTrades() {
    if (!marketAddress || isCustomMarket) {
      return null;
    }
    return await SolapeApi.getRecentTrades(marketAddress);
  }

  return useAsyncData(
    getSolapeTrades,
    tuple('getSolapeTrades', marketAddress),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
    false,
  );
}
...
export function useOpenOrdersAccounts(fast = false) {
  // console.log('useOpenOrdersAccounts: ', fast);
  fast = false;
  const { market } = useMarket();
  ...
  return useAsyncData<OpenOrders[] | null>(
    ...
    { refreshInterval: fast ? _FAST_REFRESH_INTERVAL : _VERY_SLOW_REFRESH_INTERVAL },
  );
}
...
export function useAllSelectedOpenOrdersAccount(fast = false) {
  const [accounts] = useOpenOrdersAccounts(fast);
  if (!accounts) {
    return null;
  }
...
export function useTokenAccounts(): [
  ...
  async function getTokenAccounts() {
    ...
    return await getTokenAccountInfo(connection, wallet.adapter.publicKey);
  }
  return useAsyncData(
    getTokenAccounts,
...
export function getSelectedTokenAccountForMint(
  ...
  try {
    ...
    // Errors when a key is not a pubkey
  }
}
...
export function useFillsApi() {
  ...
  async function getSolapeRecentTrades() {
    if (!market || !wallet) {
      return null;
    }

    const orderAccount = openOrdersAccounts.map(oo => oo.address.toBase58());
    const data = await SolapeApi.getOwnFills(marketName!, orderAccount);
    return data;
  }

  return useAsyncData(
    getSolapeRecentTrades,
    tuple('useFillsApi', `${marketName}#${wallet?.adapter.publicKey}`),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
    false,
  );
...
export function useFills(limit = 10000) {
  const { marketName } = useMarket();
  const fills = _useUnfilteredTrades(limit);
  // console.log('fills: ', fills);
  const [openOrdersAccounts] = useOpenOrdersAccounts();
  //@ts-ignore
  // console.log('openOrdersAccounts: ', openOrdersAccounts[0].publicKey.toBase58());
    ...
    return (
      await Promise.all(
        programIds.map((programId) =>
          OpenOrders.findForOwner(connection, wallet.adapter.publicKey, programId),
        ),
      )
    ).flat();
  };
...
export function useFreshOpenOrdersAccounts(openOrdersAccounts: OpenOrders[]) {
  const connection = useConnection();

  async function getMultipleOpenOrders() {
    if (!openOrdersAccounts.length) {
      return null;
    }
    const openOrdersAddresses = openOrdersAccounts.map(oo => oo.address);
    const freshOpenOrders = await connection.getMultipleParsedAccounts(openOrdersAddresses);

    const parsedOpenOrders = freshOpenOrders.value.map((oo: any, index: number) => {
      const openOrder = OpenOrders
        .getLayout(new PublicKey("srmqPvymJeFKQ4zGQed1GFppgkRHL9kaELCbyksJtPX"))
        .decode(oo.data);
      openOrder.address = new PublicKey(openOrdersAddresses[index]);
      return openOrder;
    });

    console.debug('openOrdersAccount: ', parsedOpenOrders);

    return parsedOpenOrders;
  }

  return useAsyncData<OpenOrders[] | null>(
    getMultipleOpenOrders,
    tuple('getMultipleOpenOrders', openOrdersAccounts),
...
export function useBalances(): Balances[] {
  ...
  const openOrdersGPA = useSelectedOpenOrdersAccount();
  const [openOrders, isLoaded] = useFreshOpenOrdersAccounts(openOrdersGPA);
    ...
    return [
      {
        ...
        orders: null,
        openOrders: openOrders && openOrders[0],
        unsettled: null,
      },
      {
        market,
  ...
  const baseExists = openOrders?.reduce((accu, next) => {
    return accu + Number(openOrders && next.baseTokenTotal && next.baseTokenFree);
  }, 0);
  // const baseExists = 
  // openOrders && openOrders.baseTokenTotal && openOrders.baseTokenFree;

  const quoteExists = openOrders?.reduce((accu, next) => {
    return accu + Number(openOrders && next.quoteTokenTotal && next.quoteTokenFree);
  }, 0);
  // const quoteExists =
  ...
  return [
    {
      ...
      orders:
        baseExists && market && openOrders
          ? market.baseSplSizeToNumber(
            openOrders.reduce((accu, oo) => {
              return accu.add(oo.baseTokenTotal.sub(oo.baseTokenFree))
            }, new BN(0))
          )
          : null,
      openOrders: openOrders && openOrders[0],
      unsettled:
        baseExists && market && openOrders
          ? market.baseSplSizeToNumber(
            // openOrders[0].baseTokenFree
            openOrders.reduce((accu, oo) => {
              return accu.add(oo.baseTokenFree)
            }, new BN(0))
          )
          : null,
    },
      ...
      orders:
        quoteExists && market && openOrders
          ? market.quoteSplSizeToNumber(
            openOrders[0].quoteTokenTotal.sub(openOrders[0].quoteTokenFree),
          )
          : null,
      unsettled:
        quoteExists && market && openOrders
          ? market.quoteSplSizeToNumber(openOrders[0].quoteTokenFree)
          : null,
    },
  ];
}
...
export function useDailyGainers(isSmall) {
  ...
  async function getSolapePairs() {
    ...
    let data = await SolapeApi.getPairs();
    // @ts-ignore
    let pairVolume = data?.sort((a: any, b: any) => {
      return b.price_change_24h - a.price_change_24h;
    }).slice(0, listLength);

    // Only show two by two for smaller screens
    let groupSize = isSmall ? 2 : 3;

    let orderedList = pairVolume?.reduce((accu: Array<any>, el: any, index) => {
      return (index % groupSize
        ? accu[accu.length - 1].push(el)
        : accu.push([el])
      ) && accu
    }, []);
...
export function useMarketData() {
  ...
  return [
    {
      key: `Solape`,
      solscanLink: `https://solscan.io/token/GHvFFSZ9BctWsEc5nujR1MTmmJWY7tgQz2AXE6WVFtGN`,
      supply: 300000000,
      decimals: 9,
      twitterLink: `https://twitter.com/SolapeFinance`,
      discordLink: `https://discord.gg/solape`,
      telegramLink: `https://t.me/SolApeOfficial`,
  ...
  async function getOpenOrdersForDeprecatedMarkets() {
    ...
    const getOrders = async (market: Market | null) => {
      ...
      try {
        ...
        return (
          await market.loadOrdersForOwner(connection, wallet.adapter.publicKey)
